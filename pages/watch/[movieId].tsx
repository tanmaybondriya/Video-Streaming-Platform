...
const Watch = () => {
  ...
  return (
    ...
    >
      ...
      {/* Top Bar */}
      <div
        className={`absolute top-0 left-0 right-0 px-6 py-6 flex items-center gap-4 bg-gradient-to-b from-black/80 to-transparent transition-opacity duration-300 ${
      ...
      {/* Bottom Controls */}
      <div
        className={`absolute bottom-0 left-0 right-0 px-6 pb-6 pt-16 bg-gradient-to-t from-black/80 to-transparent transition-opacity duration-300 ${
          showControls ? "opacity-100" : "opacity-0 pointer-events-none"
        }`}
      ...
      >
        ...
        <input
          ...
          value={progress}
          onChange={handleSeek}
          className="w-full h-1 accent-red-600 cursor-pointer mb-4"
        />
                ...
                <div className="absolute bottom-10 right-0 bg-black/90 border border-white/20 rounded-md overflow-hidden w-28">
                  ...
                  {(["1080p", "720p", "480p"] as Resolution[]).map((res) => (
                    <button
                      ...
                      className={`w-full text-left px-4 py-2 text-sm transition hover:bg-white/10 flex items-center justify-between ${
                        resolution === res
